...
static Vision_Send_s vision_send_data __attribute__((unused));  // 视觉发送数据
...
static void RemoteControlSet()
{
    ...
    { // 按照摇杆的输出大小进行角度增量,增益系数需调整
        // 死区:摇杆静态偏差(<50)不累加,防止松手时yaw/pitch目标慢慢漂移(实测rocker_l1松手有-10~-20偏差)
        if (rc_data[TEMP].rc.rocker_l_ > 50 || rc_data[TEMP].rc.rocker_l_ < -50)
            gimbal_cmd_send.yaw += 0.005f * (float)rc_data[TEMP].rc.rocker_l_;
        // pitch目标偏置(相对标定水平位PITCH_HORIZON_ECD的机械角度数):左摇杆竖直(rocker_l1)累加.
        // gimbal侧PitchReturnToHorizon以该值为相对水平偏置,目标=水平位+偏置,钳位到[水平+MIN,水平+MAX].
        // 松手时rocker_l1≈0,偏置保持,pitch停在拨到位置(不自动回水平).死区<50不累加防漂移.系数0.001:满杆660→200Hz约132°/s.
        if (rc_data[TEMP].rc.rocker_l1 > 50 || rc_data[TEMP].rc.rocker_l1 < -50)
...
{
    // 键盘按下为1, 满键对应电机端 1500 °/s (与 speed_PID.Measure 同量纲)
    chassis_cmd_send.vx = (float)(rc_data[TEMP].key[KEY_PRESS].w - rc_data[TEMP].key[KEY_PRESS].s) * 1500.0f;
    chassis_cmd_send.vy = (float)(rc_data[TEMP].key[KEY_PRESS].d - rc_data[TEMP].key[KEY_PRESS].a) * 1500.0f;
...
{
    // 记录上一tick遥控器在线状态,用于离线跳变时只打印一次,避免200Hz刷屏
    static uint8_t rc_was_online = 1;
    ...
    if (!rc_online)
    {
        ...
        shoot_cmd_send.friction_mode = FRICTION_OFF;
        shoot_cmd_send.load_mode = LOAD_STOP;
        if (rc_was_online) // 仅在线→离线跳变时打印
            LOGERROR("[rc]DT7 Dismiss");
        rc_was_online = 0;
        return; // 离线期间不进行恢复判断
    }
    rc_was_online = 1;
