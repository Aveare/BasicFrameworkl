...
#define CHASSIS_SPEED_PID_KD 0.01f
...
static void LimitChassisOutput()
{
    ...
    // 完成功率限制后进行电机参考输入设定
    if (motor_lf) DJIMotorSetRef(motor_lf, vt_lf);
    if (motor_rf) DJIMotorSetRef(motor_rf, vt_rf);
    if (motor_lb) DJIMotorSetRef(motor_lb, vt_lb);
    if (motor_rb) DJIMotorSetRef(motor_rb, vt_rb);
}
...
void ChassisTask()
{
    ...
    // 功率限制: 裁判系统未连接(chassis_power_limit为0)时,使用默认50W,否则使用裁判系统下发的限制值
    SetPowerLimit(referee_data->GameRobotState.chassis_power_limit > 0 ? referee_data->GameRobotState.chassis_power_limit : 50.0f);
    if (chassis_cmd_recv.chassis_mode == CHASSIS_ZERO_FORCE)
    { // 如果出现重要模块离线或遥控器设置为急停,让电机停止
        if (motor_lf) DJIMotorStop(motor_lf);
        if (motor_rf) DJIMotorStop(motor_rf);
        if (motor_lb) DJIMotorStop(motor_lb);
        if (motor_rb) DJIMotorStop(motor_rb);
    }
    else
    { // 正常工作
        if (motor_lf) DJIMotorEnable(motor_lf);
        if (motor_rf) DJIMotorEnable(motor_rf);
        if (motor_lb) DJIMotorEnable(motor_lb);
        if (motor_rb) DJIMotorEnable(motor_rb);
    }
    ...
    VofaSendChassisPID(); // VOFA+ 打印 4 路 3508 速度环 PID (USART1)
}
