...
static float PowerControlSpeedRpm(const DJI_Motor_Measure_s *measure)
{
    return measure->speed_aps / RPM_2_ANGLE_PER_SEC;
}
/**
...
void PowerControl()
{
    ...
    float pid_measure, power_speed_rpm, pid_ref; // 电机PID测量值、功率模型速度和设定值
    ...
    {                                // 减小访存开销,先保存指针引用
        ...
        if ((motor_setting->close_loop_type & SPEED_LOOP) && (motor_setting->outer_loop_type & (ANGLE_LOOP | SPEED_LOOP)))
        {
            ...
            power_speed_rpm = PowerControlSpeedRpm(measure);
            B = K2[i] * power_speed_rpm * power_speed_rpm;
            C = POWER_COEF * power_speed_rpm * initial_torque[i] * TORQUE_COEF;
    ...
    if (initial_total_power > chassis_max_power)
    {
        ...
        for (uint8_t i = 0; i < idx; i++)
        {
            ...
            float a = K1[i];
            float b = TORQUE_COEF * POWER_COEF * power_speed_rpm;
            float c = K2[i] * power_speed_rpm * power_speed_rpm - initial_give_power[i] + constant[i];
